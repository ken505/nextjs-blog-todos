...
export default function Auth() {
  ...
  const login = async () => {
    ...
    try {
      ...
      // 例外処理を catch で取得。
      // 認証 res が 400 番で認証が失敗した場合、authentication failed が alert で出力される。
    } catch (err) {
      alert(err);
    }
  };

  // form で submit button が押された時に呼び出される関数。
  const authUser = async (e) => {

    // リロードしないようにする。
    e.preventDefault();

    // login モードの場合、 login 関数を呼び出す。
    if (isLogin) {
      login();

      // login モードでない場合、レジスターモードで新規ユーザーを作成。
    } else {
